...
UndoPagePartialRecSize(UndoPageHeader phdr)
{
	...
	/*
	 * If record offset is not set then we don't have the partial record in
	 * the page.
	 */
	if (phdr->record_offset)
		return 0;
...
/*
 * Update the transaction compression info.
 */
static void
UpdateCompressionInfo(UndoCompressionInfo *compression_info,
					  ...
					  Buffer buffer)
{
	...
	/*
	 * If we have valid compression information then this is just a block
	 * switch so we need to set the compression info.  Otherwise we need to
	 * detect whether it's first complete record or not.  If the record is
	 * starting at just after the page header then this is the first complete
	 * record. Otherwise, we need to compute the size of the partial record
	 * and identify whether this is the first complete record after the
	 * partial record or not.
	 */
	if (compression_info->valid)
	{
		/* We should never come here if the block is not switched. */
		Assert(compression_info->blkno != UndoRecPtrGetBlockNum(urp));
	...
	/*
	 * If we are writing the first complete undo record for the page the we
	 * can set the compression and the subsequent records can avoid including
	 * common information in the undo records.
	 */
	if (first_complete_undo)
	{
		/* Set this information. */
		compression_info->rmid = urec->uur_rmid;
		compression_info->reloid = urec->uur_reloid;
		compression_info->cid = urec->uur_cid;

		/* Set that we have valid compression info. */
		compression_info->valid = true;
	}
}
				  ...
				  Oid dbid)
{
	...
	if (InRecovery)
	{
		 ...
		 */
		Assert(category != UNDO_TEMP && category != UNDO_UNLOGGED);
	...
	if (UndoRecPtrIsValid(prevlog_xact_start))
	{
		...
		/*
		 * If undo log is switched then invalidate the compression info
		 * because we need to identify whether we can initialize the
		 * compression info for this insert record or not based on which
		 * location we are inserting on the page.  Only if we are inserting
		 * the first complete record on the page we can mark it valid again
		 * otherwise it can not be reused for the subsequent records.
		 */
	...
	/*
	 * If we have valid compression info for same transaction and the current
	 * undo record is on the same block as the last undo record then exclude
	 * the common information which are same as first complete record on the
	 * page.
	 */
	if (compression_info->valid &&
		UndoRecPtrGetBlockNum(urecptr) == compression_info->blkno)
	{
	...
	/* Recompute the expected record size if we have change the undo record. */
	if (resize)
		size = UndoRecordExpectedSize(urec);
	...
	/*
	 * If we have valid compression info for the transaction and it's for the
	 * same block in which we are inserting the current undo then we don't
	 * need to change the compression info.
	 */
	if (!compression_info->valid ||
		compression_info->blkno != UndoRecPtrGetBlockNum(urecptr))
	{
		int			index = prepared_undo->undo_buffer_idx[0];

		/*
		 * Pass the buffer from which we are writing the current undo record.
		 * This might required to identify whether this is the first complete
		 * record on the page or not.
		 */
		UpdateCompressionInfo(compression_info, urec, urecptr,
							  context->prepared_undo_buffers[index].buf);
	}
